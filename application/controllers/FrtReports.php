...
class FrtReports extends CI_Controller {
	...
	function __construct() { 
		...
		$this->load->model(array('settings_model','tasks_model','common_model','reports_model','frtreports_model'));
	}
		...
		$data['managers_list']=$managers_list->data;
						...
						$this->load->view('frtreports/frtreports',$data);
		...
		{ 
			...
			$task_dump_report= json_decode($this->frtreports_model->get_task_dump_report_pdf());
			...
			$task_page=$this->load->view('frtreports/task_dump_report_pdf',$data,TRUE);
			...
			$this->m_pdf->pdf->Output($pdfFilePath, "I"); 				
				...
				if(isset($_POST['report_type']) && $_POST['report_type']==1){
					$employee_data= json_decode($this->frtreports_model->get_attendanceports());
					...
					$task_dump_reportdata= json_decode($this->frtreports_model->get_task_dump_reportdata());
					...
					header('Content-Disposition: attachment; filename='.$fname.'');  
					$output = fopen("php://output", "w");  
					fputcsv($output, array('sno','Employee Name','Team Type','Employee Identity','Client Name','Assignment Identity','Order Number','Service Number','Task was Auto Assigned','Assignment Type','Reason For Auto assignment not done','Auto assignment Changed By Manager','Approver','Task Coordinates','Travel Start Coordinates','Travel Start Address','Travel End Coordinates','Travel End Address','Distance Travelled (Km)','Is Travelled','Reason_for_distance_not_capturing','Return Distance Travelled (Km)','Reason_for_return_distance_not_capturing','Recorded Hardware','Hardware Description','Unit / Meter','Pincode','Task Type','Creation On','Priority','Address','Timestamp','Status','Mobile Numbe','Cluster','Circle','Region','City_Type','Dispatcher','Vendor','Vendor Code','Designation','Resource Type','Acknowledge Time','Travel Start Time','Onsite Time','Completion Time','Approved Remarks'));  
					$fields=array();
					...
					$task_performancereportdata= json_decode($this->frtreports_model->get_task_performancereport());
					...
					$employee_data= json_decode($this->frtreports_model->getemployeereports());
					$employee_data=$employee_data->data;
